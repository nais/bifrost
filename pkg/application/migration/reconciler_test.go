...
func (m *MockUnleashRepository) List(ctx context.Context, excludeChannelInstances bool) ([]*unleash.Instance, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockUnleashRepository) AddInstance(name, customVersion, releaseChannel string, isReady bool) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockUnleashRepository) SetReadyAfterNCalls(name string, n int) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.readyAfter[name] = n
}
...
func newTestReconciler(repo unleash.Repository, channelRepo releasechannel.Repository, cfg *config.Config, logger *logrus.Logger) *Reconciler {
...
func TestWaitForHealthy_ImmediateReady(t *testing.T) {
	repo := NewMockUnleashRepository()
	...
	err := waitForHealthy(ctx, repo, logger, "test-instance", 5*time.Second, 10*time.Millisecond)
	assert.NoError(t, err)
}

func TestWaitForHealthy_EventuallyReady(t *testing.T) {
	repo := NewMockUnleashRepository()
...
func TestLogCurrentState(t *testing.T) {
	...
	state.Store("instance-1", &instanceState{
		originalValue: "6.1.0",
		status:        statusCompleted,
	})
	state.Store("instance-2", &instanceState{
		originalValue: "6.2.0",
		status:        statusInProgress,
	})

	pending := newPendingQueue()
	pending.add("instance-3", "instance-4")

	// Should not panic
	logCurrentState(logger, &state, pending, "test reason")
}
...
func TestPendingQueue_Remove(t *testing.T) {
	...
	pending.remove("b")
	assert.Equal(t, []string{"a", "c", "d"}, pending.copy())

	pending.remove("a")
	assert.Equal(t, []string{"c", "d"}, pending.copy())

	pending.remove("d")
	assert.Equal(t, []string{"c"}, pending.copy())

	pending.remove("nonexistent")
	assert.Equal(t, []string{"c"}, pending.copy())
}
